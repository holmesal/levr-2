	...
	def post(self):
		try:
			logging.info('''
				...
				THE FOURSQUARE DEAL SEARCH TASK IS RUNNING. Jk. No it's not anymore
...
class BusinessHarmonizationTaskHandler(webapp2.RequestHandler):
	def post(self):
		try:
			...
			THE FOURSQUARE BUSINESS TASK IS RUNNING. Jk. No it's not anymore
...
class FoursquareDealUpdateTaskHandler(webapp2.RequestHandler):
	def post(self):
		try:
			...
			THE FOURSQUARE DEAL UPDATE TASK IS RUNNING. Jk no its not anymore.
